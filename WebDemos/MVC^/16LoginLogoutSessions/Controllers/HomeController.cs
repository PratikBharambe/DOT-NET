...
namespace _16LoginLogoutSessions.Controllers
{
    public class HomeController : Controller
    {
        ...
        public HomeController(HelloContext context)
        {
            _context = context;
        }
        ...
        {
            var authUser = _context.User.Where(u => u.Name == user.Name && u.Email == user.Email && u.Password == user.Password).FirstOrDefault<User>();
            if (authUser != null)
            {
                HttpContext.Session.SetString("authUserName", authUser.Name);
                return RedirectToAction("Dashboard");
            }
            else
            {
